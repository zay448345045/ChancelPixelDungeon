...
public class Badges
{
	...
	public static void validateItemLevelAquired(Item item)
	{
		...
		if(level < 3 || item instanceof Artifact)
		{
			return;
		}

		Badge badge = null;
		if(!local.contains(Badge.ITEM_LEVEL_1) && level >= 3)
		{
			badge = Badge.ITEM_LEVEL_1;
			local.add(badge);
		}
		if(!local.contains(Badge.ITEM_LEVEL_2) && level >= 6)
		{
			badge = Badge.ITEM_LEVEL_2;
			local.add(badge);
		}
		if(!local.contains(Badge.ITEM_LEVEL_3) && level >= 9)
		{
			badge = Badge.ITEM_LEVEL_3;
			local.add(badge);
		}
		if(!local.contains(Badge.ITEM_LEVEL_4) && level >= 12)
		{
