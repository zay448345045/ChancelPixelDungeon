...
public class StatusPane extends Component
{
	...
	private Emitter dynamic;
	private int dynaSpeck = -1;
	...
	protected void createChildren()
	{
		...
		dynamic = new Emitter();
		dynamic.pos(avatar.width, avatar.height + 4f);
		dynamic.autoKill = false;
		dynamic.fillTarget = false;
		updateDynamic();
		add(dynamic);
	...
	private void updateDynamic()
	{
		...
		if(dyna < Hero.DYNAMIC_MILESTONES[0])
			...
			speck = Speck.WHITE_LIGHT;
		else if(dyna < Hero.DYNAMIC_MILESTONES[2])
			speck = Speck.YELLOW_LIGHT;
		else if(dyna < Hero.DYNAMIC_MILESTONES[3])
			speck = Speck.ORANGE_LIGHT;
		else if(dyna < Hero.DYNAMIC_MILESTONES[4])
			speck = Speck.RED_LIGHT;
		else
			speck = Speck.PURPLE_LIGHT;

		if(speck != dynaSpeck)
		{
			dynaSpeck = speck;
			dynamic.pour(Speck.factory(speck), 0.6f);
			dynamic.on = speck != 0;
		}
	}
