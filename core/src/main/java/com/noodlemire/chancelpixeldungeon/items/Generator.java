...
public class Generator
{
	public enum Category
	{
		...
		static
		{
			...
			SCROLL.classes = new Class<?>[]{
					...
					ScrollOfTaunt.class,
					ScrollOfMagma.class};
			SCROLL.probs = new float[]{30, 10, 20, 0, 15, 12, 8, 8, 1, 4, 10, 6, 10, 12, 8, 4, 7, 10, 12, 10};
			...
			STONE.classes = new Class<?>[]{
					...
					StoneOfBinding.class,
					StoneOfBlast.class,
					StoneOfHypnotism.class,
					StoneOfIntuition.class,
					StoneOfPreservation.class,
					StoneOfShock.class,
					StoneOfFlock.class,
	...
	public static Scroll randomScroll()
	{
		try
		{
			Category cat = Category.SCROLL;
			Class<? extends Scroll> scr;
			Scroll scroll;

			do
			{
				scr = (Class<? extends Scroll>) cat.classes[Random.chances(cat.probs)];
				scroll = scr.newInstance();
			}
			while(scroll.image == ItemSpriteSheet.SCROLL_MYSTERY);
	...
	{
		return randomRing(true);
	}

	//Set this to false to prevent random but not randomly upgraded rings from
	//preventing other wands/rings from randomly upgrading
	public static Ring randomRing(boolean upgrade)
	{
		try
		{
			Ring ring = (Ring) Category.RING.classes[Random.chances(Category.RING.probs)].newInstance();
			//limits the amount of upgraded wands or rings the player can find
			int limit = 3 * (Dungeon.depth / 5 + 1);
			int count = Dungeon.LimitedDrops.MISC_LEVELS.count;

			if(count < limit && upgrade)
			{
				int max = Math.min(2, limit - count);
				int lvl = Random.IntRange(0, max);

				if(lvl > 0)
				{
					ring.level(lvl);
					Dungeon.LimitedDrops.MISC_LEVELS.count += lvl;
				}
			}
	...
	{
		return randomWand(true);
	}

	//Set this to false to prevent random but not randomly upgraded wands from
	//preventing other wands/rings from randomly upgrading
	public static Wand randomWand(boolean upgrade)
	{
		try
		{
			Wand wand = (Wand) Category.WAND.classes[Random.chances(Category.WAND.probs)].newInstance();
			//limits the amount of upgraded wands or rings the player can find
			int limit = 3 * (Dungeon.depth / 5 + 1);
			int count = Dungeon.LimitedDrops.MISC_LEVELS.count;
