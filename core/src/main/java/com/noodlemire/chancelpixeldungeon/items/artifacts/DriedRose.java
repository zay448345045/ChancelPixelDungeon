...
import com.noodlemire.chancelpixeldungeon.windows.WndBlacksmith;
...
public class DriedRose extends Artifact
{
	...
	public static class GhostHero extends NPC
	{
		...
		{
			updateRose();
			if(rose == null || !rose.isEquipped(Dungeon.hero) || (rose.cursed && !rose.isBound()))
				damage(1, this);
					...
					else
					{
						GameScene.selectItem(new WndBag.Listener()
						{
							...
							public void onSelect(Item item)
							{
								...
								else if(item.cursed && !item.isBound())
								{
