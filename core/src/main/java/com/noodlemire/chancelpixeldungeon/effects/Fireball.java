...
public class Fireball extends Component {
	...
	private static final int COLOR = 0x666666;
	...
	protected void createChildren() {
		...
		emitter.pour( new Emitter.Factory() {
			...
			public void emit(Emitter emitter, int index, float x, float y) {
				...
				p.hspeed = (-Camera.main.width / 2 + x) * 2;
				p.reset();
				p.x = x - p.width / 2;
				p.y = y - p.height / 2;
			}
	...
	public void update() {
		...
		if (Random.Float() < Game.elapsed) {
			...
			spark.reset( x, y, ColorMath.random( COLOR, 0xAA66AA ), 2, Random.Float( 0.5f, 1.0f ) );
			spark.speed.set(
				Random.Float( -40, +40 ),
				Random.Float( -60, +20 ) );
			spark.acc.set( 0, +80 );
	...
	public static class Flame extends Image {
		...
		private static float VSPEED = -40f;
